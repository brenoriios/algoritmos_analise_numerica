...
class InputData:
    ...
    def __init__(self, function: Function, starting_point: Decimal, stop_condition):
...
def solve_for_interval(function: Function, value: Decimal, iteration: int):
    ...
    return Solution(next_value, abs(next_value - value))
...
def newton_raphson_solve(function: Function, starting_point: Decimal, stop_condition: StopCondition):
    ...
    iteration = 1
    solution: Solution = solve_for_interval(function, starting_point, iteration)
    
    while((abs(solution.error) > stop_condition.value) and iteration <= 9999):
        iteration += 1
        solution = solve_for_interval(function, solution.value, iteration)
