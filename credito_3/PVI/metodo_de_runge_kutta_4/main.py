...
def get_next_y(point: Point, function: Function, h: Decimal):
    ...
    k2 = solve_function(function, [point.x + (Decimal(1 / 2) * h), point.y + (Decimal(1 / 2) * k1 * h)])
    k3 = solve_function(function, [point.x + (Decimal(1 / 2) * h), point.y + (Decimal(1 / 2) * k2 * h)])
    k4 = solve_function(function, [point.x + h, point.y + (k3 * h)])

    next_y = point.y + (Decimal(1 / 6) * (k1 + (2 * k2) + (2 * k3) + k4) * h)
