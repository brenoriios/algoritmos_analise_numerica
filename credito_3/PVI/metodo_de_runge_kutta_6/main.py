...
def get_next_y(point: Point, function: Function, h: Decimal):
    ...
    k2 = solve_function(function, [point.x + (Decimal(1 / 4) * h), point.y + (Decimal(1 / 4) * k1 * h)])
    k3 = solve_function(function, [point.x + (Decimal(1 / 4) * h), point.y + (Decimal(1 / 8) * k1 * h) + (Decimal(1 / 8) * k2 * h)])
    k4 = solve_function(function, [point.x + (Decimal(1 / 2) * h), point.y - (Decimal(1 / 2) * k2 * h) + (k3 * h)])
    k5 = solve_function(function, [point.x + (Decimal(3 / 4) * h), point.y + (Decimal(3 / 16) * k1 * h) + (Decimal(9 / 16) * k4 * h)])
    k6 = solve_function(function, [point.x + (Decimal(3 / 4) * h), point.y - (Decimal(3 / 7) * k1 * h) + (Decimal(2 / 7) * k2 * h) + (Decimal(12 / 7) * k3 * h) - (Decimal(12 / 7) * k4 * h) + (Decimal(8 / 7) * k5 * h)])

    next_y = point.y + (Decimal(1 / 90) * ((7 * k1) + (32 * k3) + (12 * k4) + (32 * k5) + (7 * k6)) * h)
